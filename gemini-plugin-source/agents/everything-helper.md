...
3. Recommendations: Suggest specific TwinEngine packs and agents.
4. Call to Action: Tell the user exactly which command to run (e.g., /bootstrap).

## Discovery Step
Always begin by reading `~/.gemini/.twinengine_source` to identify the correct path to the TwinEngine repository on this machine. Then use that path to look for agents in `gemini-plugin-source/agents/`.
